...
/**
 * Demonstrates the use of ServerlessPHPEnvMagic.
 */
function main()
{
    $serverlessEnvMagic = new ServerlessPHPEnvMagic();
    $serverlessEnvMagic->setConstant(); // Sets the SRVLS_PHP_ENV_MGK constant
    
    // Assuming an environment variable LAMBDA_MEMORY_SIZE holds the memory size in MB
    $lambdaMemorySize = getenv('LAMBDA_MEMORY_SIZE') ?: '0'; // Default to '0' if not set

    // Convert to int for comparison
    $lambdaMemorySizeMb = (int)$lambdaMemorySize;

    // Check if Lambda has more than 768 MB of RAM
    if ($lambdaMemorySizeMb > 768) {
        // Get environment with file contents if condition is met
        $envData = $serverlessEnvMagic->getEnvironment(true);
    } else {
        // Otherwise, get the base environment data
        $envData = $serverlessEnvMagic->getEnvironment();
    }

    // Convert the data to JSON for output
    $envDetailsJson = json_encode($envData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);

    // Echo the JSON string
    echo $envDetailsJson;

    // If part of a web application, you might return this data instead
    return ['body' => $envDetailsJson];
}

// Execute the main function to use the class
main();
